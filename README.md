...
**Next up (in order):** the assessment pipeline — `createAssessmentUploadUrl` (presigned S3 POST with size/MIME limits) → `submitAssessment` → Horizon job (Deepgram batch → DeepSeek analysis → CEFR level → `RoadmapGenerator::regenerate()`) → `assessmentReady` subscription (plan §6, October weeks 3–4, second half). Then the Nuxt roadmap screen, which also needs the Sanctum double-submit handshake wired into the urql client.
