...
**Try it locally:** `docker compose up -d`, then `php artisan serve` + `php artisan queue:work` + `php artisan reverb:start` in `backend/` and `npm run dev` in `frontend/`. The seeded account `test@example.com` / `password` signs in at <http://localhost:3000>; **Проверить уровень** records a minute of speech, sends it through the real pipeline and rewrites the roadmap from the level that comes back.
