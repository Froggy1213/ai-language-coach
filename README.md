...
- **Frontend**: Nuxt in SPA mode (`ssr: false` — the session cookie lives in the browser, so there is nothing to render on the server), Tailwind v4 via `@tailwindcss/vite` (`app/assets/css/main.css`), and a global urql client as `$urql` (`app/plugins/urql.ts`). The client carries the **Sanctum double-submit handshake**: it fetches `GET /sanctum/csrf-cookie` when the cookie is missing (deduplicated, so parallel queries share one request) and re-reads the `XSRF-TOKEN` cookie into an `X-XSRF-TOKEN` header on every request — `login` regenerates the session, so a cached header is what turns the next mutation into a 419. Pages: `/login`, `/register`, `/roadmap`; `app/middleware/auth.global.ts` resolves the session once per page load and bounces guests to `/login`; `useAuth()` (`app/composables/useAuth.ts`) keeps `me` in `useState` for the header, the guard and the pages. UI copy is Russian, matching the product audience. **Not wired yet**: subscriptions (they need the Echo/pusher-js transport, not `graphql-ws`) and the recording UI — both arrive with the onboarding screen, which also needs a bucket.
...
- **Assessment pipeline**: `createAssessmentUploadUrl` presigns a limited S3 POST under `assessments/{userId}/`, `submitAssessment` re-checks the stored object (owner, ≤15 MB, audio content type) and queues `App\Assessments\AnalyzeAssessment`. The job runs Deepgram batch STT (`App\Assessments\DeepgramTranscriber`), then the CEFR judgement (`App\Assessments\DeepSeekCefrAssessor`), writes `assessments.status`/`cefr_level`/`raw_data`, moves `users.current_level`, regenerates the roadmap, deletes the recording and broadcasts `assessmentReady`. Needs `DEEPGRAM_API_KEY`, `DEEPSEEK_API_KEY`, `AWS_BUCKET` + `AWS_*` credentials (all empty in `.env.example`, so the pipeline only runs once they are filled in); limits live in `config/assessments.php`.
- **Tests**: Pest 5 + PHPUnit classes, `LazilyRefreshDatabase` against `language_coach_testing`, `php artisan test` green — 110 tests, including the S3 policy limits, the Deepgram request/response shape, the job's success, skip, retry and terminal-failure paths, and the subscription's ownership checks.
...
**Try it locally:** with MySQL/Redis up and both dev servers running, the seeded account `test@example.com` / `password` signs in at <http://localhost:3000> and already has an A1 roadmap (6 cards) to look at; a fresh registration lands on the empty state with a **Собрать роадмап** button. `DEEPGRAM_API_KEY` and `DEEPSEEK_API_KEY` are wired, but the assessment pipeline also needs `AWS_BUCKET` + credentials, so the recording screen waits for those.

**Next up (in order):** the onboarding recording screen — `MediaRecorder` → presigned POST → `submitAssessment`, then `assessmentReady` over the Echo/pusher-js transport (the SPA needs `NUXT_PUBLIC_REVERB_APP_KEY` for that), plus the consent step plan §5 requires before recording. It needs a bucket, so it is the natural pairing with the AWS work. After that the roadmap screen grows practice links once `requestVoiceToken` and the voice agent exist (November).
