...
20. **The urql client is installed into the Vue app, not only into Nuxt's global properties.** `app/plugins/urql.ts` calls `installUrql(nuxtApp.vueApp, client)` *and* keeps `provide: { urql }`. Both are needed, because `@urql/vue` reads the client from two places that never see each other: Nuxt's `provide` sets `$urql` as a global property (which `useAuth()` and `useAssessment()` call directly), while `useQuery()` resolves through Vue's `provide`/`inject` and throws "No urql Client was provided" without the install. The roadmap screen was the only `useQuery()` caller, so the symptom was a browser that signed in successfully and then rendered a 500 — `login` → 200, `me` → 200, and no `roadmap` request at all, exactly the shape the plan's open defect recorded.
21. **`config/cors.php` matches `graphql/*`, the recording screen waits for the channel to be subscribed, and the listener reads Lighthouse's `{more, result}` envelope.** Three defects on one path, none of them visible from the server side. Laravel's CORS middleware matches paths with `Str::is()`, so a bare `graphql` covers `/graphql` exactly and not `/graphql/subscriptions/auth` — the one request Echo's custom authorizer makes. The browser discarded that response, pusher-js reported "Failed to fetch", `pusher:subscribe` was never sent, and the `assessmentReady` push had nowhere to land: the assessment completed, the roadmap regenerated, and the waiting screen sat there. `useAssessment().onReady()` now awaits Echo's own `channel.subscribed()` confirmation (10 s) instead of assuming the handshake worked, so a refused origin, a 419 or a missing Reverb surfaces on the screen rather than as an indefinite wait. And `PusherBroadcaster::broadcast()` triggers `{more: true, result: <execution result>}`, so the payload is at `result.data.assessmentReady` — the listener subscribed to `.lighthouse-subscription` correctly but read `payload.data`, which is always undefined, so even a delivered push left the screen unchanged. `AssessmentReadyChannelTest` pins the CORS header, because that assertion has to be on the response a browser receives — a server-side test cannot see a header a browser needs.
...
**Try it locally:** `docker compose up -d`, then `php artisan serve` + `php artisan queue:work` + `php artisan reverb:start` in `backend/` and `npm run dev` in `frontend/`. The seeded account is `test@example.com` / `password`; signing in reaches the roadmap, and **Проверить уровень** records a minute of speech, sends it through the real pipeline and rewrites the roadmap from the level that comes back. Both paths were walked in a browser on 24.09 (headless Chrome, a synthetic recording as the microphone); the three defects that walk surfaced — the urql client never reaching Vue's injection, the channel-authorization route sitting outside the CORS path list, and the subscription listener reading the wrong envelope — are recorded as decisions 20 and 21.
